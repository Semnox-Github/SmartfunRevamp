...
class PosMachineViewRespository {
  ...
  Future<List<PosMachineDTO>?> getPosMachineViewDTOList(
    ...
    List<Object>? posMachineViewDTOList = [];
    // check if the value is present in the cache or local storage
    posMachineViewDTOList = await _getLocalData(executionContext);

    // If the local data is not there, get remote data
    posMachineViewDTOList ??= await _getRemoteData(executionContext);
    return PosMachineDTO.getPosMachineDTOList(posMachineViewDTOList);
  }
